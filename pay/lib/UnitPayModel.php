...
class UnitPayModel
{
    ...
    {
	$array_account = explode("_", $account);
	
	if($array_account[2] == 0 && $array_account[2] > 2) return false;
	if($array_account[2] == 1) $lifetime = '?lifetime=2592000';
	else $lifetime = '';
	$group = $array_account[1].$lifetime;
        ...
        $query = "
            ...
            WHERE
                ".Config::TABLE_ACCOUNT_NAME." = '".$this->mysqli->real_escape_string($array_account[0])."'
        ";
        $sql = "INSERT INTO shopcart VALUES('','$array_account[0]','permgroup','$group','1',NULL)";
